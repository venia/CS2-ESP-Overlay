...
// Advanced Memory Scanner with Pattern + Heuristic Detection
...
struct Pattern {
    ...
    std::string mask;
    std::string name;
    int offset;  // смещение от найденного адреса до данных
};
...
// ============================================
// PATTERNS FOR m_vecOrigin
// ============================================

std::vector<Pattern> g_patterns = {
    // Паттерн 1: CGameSceneNode::m_vecOrigin (наиболее распространенный)
    {
        {0x48, 0x8B, 0x05, 0x00, 0x00, 0x00, 0x00, 0xF3, 0x0F, 0x10, 0x00, 0xF3, 0x0F, 0x10, 0x48, 0x00},
        "xxx????xxxxxxx?",
        "m_vecOrigin_pattern_1",
        0x03
    },
    // Паттерн 2: Альтернативный
    {
        {0x48, 0x8B, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xC9, 0x74, 0x00, 0x48, 0x8B, 0x01, 0xFF, 0x50, 0x00},
        "xxx????xxx?xxxxxx?",
        "m_vecOrigin_pattern_2",
        0x03
    },
};
...
uintptr_t FindPattern(HANDLE hProcess, uintptr_t startAddress, size_t size, 
...
uintptr_t FindPatternInRegions(HANDLE hProcess, const std::vector<MemoryRegion>& regions,
                               const std::vector<BYTE>& pattern, const std::string& mask) {
    for (const auto& region : regions) {
        uintptr_t result = FindPattern(hProcess, region.start, region.size, pattern, mask);
        if (result != 0) {
            return result;
        }
    }
...
// ============================================
// ADVANCED MEMORY SCANNING (Dynamic)
// ============================================
...
void PrintHeader() {
    std::cout << "╔══════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║     CS2 PATTERN SCANNER v4 - Pattern + Dynamic             ║" << std::endl;
    std::cout << "╚══════════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << std::endl;
}
...
int main() {
    ...
    // ============================================
    // STEP 1: PATTERN SCANNING (FIND m_vecOrigin)
    // ============================================
    
    std::cout << "[PATTERN] Searching for m_vecOrigin via patterns..." << std::endl;
    
    bool patternFound = false;
    for (const auto& pattern : g_patterns) {
        std::cout << "[PATTERN] Trying: " << pattern.name << std::endl;
        
        uintptr_t addr = FindPatternInRegions(hProcess, g_regions, pattern.bytes, pattern.mask);
        if (addr != 0) {
            uintptr_t dataAddr = addr + pattern.offset;
            uintptr_t value = 0;
            if (ReadMemory(hProcess, dataAddr, value)) {
                uintptr_t offset = value - clientBase;
                std::cout << "[PATTERN] ✓ FOUND! Address: 0x" << std::hex << addr 
                          << " | Offset: 0x" << offset << std::dec << std::endl;
                g_foundOffsets.m_vecOrigin = offset;
                patternFound = true;
                g_found = true;
                break;
            }
    ...
    // ============================================
    // STEP 2: DYNAMIC SCANNING (if pattern not found)
    // ============================================
    
    if (!patternFound) {
        std::cout << "[PATTERN] No patterns found. Falling back to dynamic scan..." << std::endl;
        std::cout << std::endl;
        
        // Fallback offsets for other fields
        ...
        // ============================================
        // PATTERN FOUND! Save immediately.
        // ============================================
        std::cout << std::endl;
        std::cout << "╔══════════════════════════════════════════════════════════════╗" << std::endl;
        std::cout << "║         m_vecOrigin FOUND via PATTERN!                     ║" << std::endl;
        std::cout << "╚══════════════════════════════════════════════════════════════╝" << std::endl;
        std::cout << "  Offset: 0x" << std::hex << g_foundOffsets.m_vecOrigin << std::dec << std::endl;
        std::cout << std::endl;
        
        // Заполняем остальные смещения (захардкожены)
        g_foundOffsets.dwEntityList = 0x2571220;
        g_foundOffsets.dwLocalPlayerPawn = 0x23C6268;
        g_foundOffsets.dwLocalPlayerController = 0x23A0F30;
        g_foundOffsets.dwViewMatrix = 0x23CB830;
